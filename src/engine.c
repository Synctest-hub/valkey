...
int engineManagerRegisterEngine(const char *engine_name,
    ...
    *e = (engine){
    ...
    engineMgr.engine_cache_memory += zmalloc_size(e) +
                                     sdsAllocSize(e->name) +
                                     zmalloc_size(ei) +
                                     ei->methods.get_engine_memory_overhead(ei->ctx);
...
compiledFunction **engineCallCreateFunctionsLibrary(
    ...
    char **err) {
    return engine->impl->methods.create_functions_library(
        engine->impl->ctx,
        code,
        timeout,
...
size_t engineCallGetUsedMemory(engine *engine) {
    return engine->impl->methods.get_used_memory(engine->impl->ctx);
}

size_t engineCallGetFunctionMemoryOverhead(engine *engine, void *compiled_function) {
    return engine->impl->methods.get_function_memory_overhead(compiled_function);
}

void engineCallFreeFunction(engine *engine,
                            void *compiled_func) {
    engine->impl->methods.free_function(engine->impl->ctx,
                                        compiled_func);
}
